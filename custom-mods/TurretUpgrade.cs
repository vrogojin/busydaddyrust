...
namespace Oxide.Plugins
{
    ...
    public class TurretUpgrade : RustPlugin
    {
        ...
        private void Init()
        {
            ...
            // Start engine state monitoring timer
            timer.Every(1f, () => CheckEngineStates());
        }
        ...
        private void OnVehicleUpgradeLoaded(ModularCar vehicle, object upgradeData)
        {
            ...
            if (turret != null)
            {
                ...
                // Delay restoration to ensure turret is fully initialized
                Puts($"[TurretUpgrade] Scheduling inventory restoration in 1 second...");
                ...
                if (itemsJArray != null)
                {
                    ...
                    foreach (var itemToken in sortedTokens)
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            // Check turret inventory slots
                            Puts($"[TurretUpgrade] Turret inventory before restore: capacity={turret.inventory.capacity}, items={turret.inventory.itemList.Count}");
                            for (int i = 0; i < turret.inventory.capacity; i++)
                            {
                                var slotItem = turret.inventory.GetSlot(i);
                                if (slotItem != null)
                                {
                                    Puts($"[TurretUpgrade] Slot {i}: {slotItem.info.shortname} x{slotItem.amount}");
                                }
                            }
                            ...
                            {
                                // Skip if weapon and position is 0 (weapon slot)
                                if (position == 0 && item.info.category == ItemCategory.Weapon)
                                {
                                    placed = item.MoveToContainer(turret.inventory, position);
                                }
                                else if (position > 0 && item.info.category == ItemCategory.Ammunition)
                                {
                                    // Try to place ammo in a different slot
                                    placed = item.MoveToContainer(turret.inventory, position);
                                }
                                else
                                {
